/*
A non-empty zero-indexed array A consisting of N numbers is given. The array is sorted in non-decreasing order. The absolute 
...
int solution(vector<int> &A) {
    ...
    while (ptr!=A.end()) {
        if (*ptr<0) {
            ...
            if (b.empty()) 
                return distinct + distance(ptr,A.end());    
