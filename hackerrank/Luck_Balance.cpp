...
int luckBalance(int k, vector<vector<int>> contests) {
  int important = count_if(contests.begin(), contests.end(), isImportant);
  int haveToWin = (important > k) ? (important - k) : 0;
  // cout << "important " << important << endl;

  int luckSum = 0;
  for (auto a : contests)
    luckSum += a[0];
  // cout << "luckSum " << luckSum << endl;
  
  while (haveToWin) {
    int smallluck = numeric_limits<int>::max();
    vector<vector<int>>::iterator todelete;
    for (vector<vector<int>>::iterator ptr = contests.begin(); ptr != contests.end(); ptr++) {
      if ((smallluck > ptr->at(0)) && (ptr->at(1))) {
        smallluck = ptr->at(0);
        todelete = ptr;
      }
    }
    luckSum -= (smallluck*2);
    contests.erase(todelete);
    haveToWin--;
  }
